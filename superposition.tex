...
\titem increase tension in string (wave speed increases\footnote{The speed $v$ of a progressive wave travelling along a string of length $L$ and mass $m$ can be given by the equation: $v = \sqrt{\frac{TL}{m}}$, where $T$ is the tension in the string.} for same frequency, so $\lambda$ increases) \eoe
