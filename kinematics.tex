...
in everyday language, we talk about the \keypoint{distance} travelled by an object, which usually refers to the length travelled by an object without considering in which direction it moves

to fully describe position of an object, we also need specify where it is heading
...
\begin{ilight}
	\centering change in displacement per unit time is called the \keypoint{velocity} of the object\index{velocity}: $\boxed{v=\frac{\Delta s}{\Delta t}}$
\end{ilight}
...
\begin{ilight}
	\centering change in velocity per unit time is defined as the \keypoint{acceleration}\index{acceleration}: $\boxed{a = \frac{\Delta v}{\Delta t}}$
\end{ilight}
...
as a consequence, a projectile would follows a \emph{parabolic} path as it travels\footnote{\ifthenelse{\includequestions=1}{You may be able to prove this statement in Question \ref{Q-projectile-eqn}.}{You may try to prove this statement with a bit algebra.}}
