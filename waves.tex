...
distance between two adjacent points undergoing exactly same motion is the \keypoint{wavelength} ($\lambda$)
...
time for a particle to complete a full oscillation cycle is the  \keypoint{period} ($T$)

\cmt number of oscillations for a particle per unit time is the \keypoint{frequency} ($f$)
...
\begin{ilight}
	a \keypoint{transverse} wave has vibrations at right angle to its direction of energy transfer
\end{ilight}
...
\begin{ilight}
	a \keypoint{longitudinal} wave has vibrations in parallel direction to energy transfer
\end{ilight}
...
two key apparatuses for sound measurement are the \keypoint{microphone} and the \keypoint{oscilloscope}
...
a wave can be either \emph{mechanical} or \emph{electromagnetic}, depending on whether it requires a medium
...
\begin{ilight}
	\keypoint{intensity} of a wave is defined as the power $P$ per unit area on a cross section $S$: $\boxed{I=\frac{P}{S}}$
\end{ilight}
...
\question{
	In our everyday lives, diffraction of sound waves is much easier to observe than diffraction of light waves. Why is this?
}
...
\question{
	A spectral line of a particular wavelength is emitted from a distant star. The wavelength observed is found to vary periodicaly from a maximum value to a minimum value and back to the maximum value. What can you say about the motion of the star?
}
